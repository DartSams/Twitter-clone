...
import json
from flask_socketio import SocketIO,emit,send,join_room, leave_room #replaces post request
load_dotenv()
...
ALLOWED_EXTENSIONS = ['txt', 'pdf', 'png', 'jpg', 'jpeg', 'gif', 'jfif'] #defining what type users can upload
...
def split_compare_date(full_date):
    ...
    if "" in full_date: #this is for single digit days like 5,6,7 are different than 10,12,13
        full_date.remove("")
...
def index():
    ...
    like_lst_id=[] #list that holds all post id that the currently signed user has liked
    ...
    if request.method=="GET":
        ...
        return render_template("index.html",data=data) #on GET request loads the home page and sends the data variable that holds all personal data to html frontend


    elif request.method=="POST":
        #print/debug request.form to get all input data from html
        post=request.form.get("post-field") #assigns the post input field from html using the assigned name 'post-field' to the python variable post
        post=post.capitalize() #capitalize all post 
        post_date=time.ctime()

        if 'file' in request.files:
            file = request.files['file'] #searches the input fields for a uploaded file
            if file and allowed_file(file.filename):
                filename = secure_filename(file.filename)
                file.save(os.path.join(app.config['UPLOAD_FOLDER'], filename)) #saves the uploaded file to the set path of UPLOAD_FOLDER
...
def profile2(username,tab):
    ...
    if request.method=="GET":    
        ...
        #if user if logged in then creates a variable for the html to display if logged in user is following the requested user
        if "username" in session:
            mycursor.execute("SELECT * FROM Follow WHERE name = %s ",session["username"])
            all_ready_followed = mycursor.fetchone()
        else:
            all_ready_followed=" "
        ...
        if myresult == None:
            ...
            flash(f"Hmm... the tab {tab} doesn’t exist. Try searching for something else.")
...
#quick way for admins to switch users to test bugs/ideas like testing if the like button works corretcly istead of liking the post for all users
@app.route("/switch/<username>")
def switch(username):
    session["username"]= username
    return redirect(f"/{username}")

#on the admin page admins can see all users and post can also promote users to admin or demote admins to user,delete users,edit db tables like change names,delete,create tables,add columns or rename columns in tables
...
def handle_message(post):
    room=post['type'] # because i reused js functions i need to seperate them into rooms so it doesnt create entries in more than 1 db table
    ...
    if post["type"] == "newPost" or post["type"] == "retweetPost":
        ...
        emit("message",post,broadcast=True,to=room)  #setting broadcast to True means if 2 users are currently on the sage page as the socket then both 
...
def changeLikes(data):
    if data["status"] == "like":
        mycursor.execute("INSERT INTO Likes (name,id) VALUES (%s,%s) ",(data["user"],data["id"]) )
        conn.commit()
    
    elif data["status"] == "unlike":
        mycursor.execute("DELETE FROM likes WHERE name = %s AND id = %s",(data["user"],data["id"]))
        conn.commit()
...
def comment(data):
    ...
    room=data["type"] #because i reused js functions i need to seperate them into rooms so it doesnt create entries in more than 1 db table
    join_room(room) # joins the room to emit a signal 
    ...
    current_post=mycursor.fetchall()
    row_headers=[x[0] for x in mycursor.description] #this will extract row headers
    data=dict(zip(row_headers,current_post[-1])) # after inserting a new entry in the db this will query that entry and the column headers to send to client side
